...
def createMoveList(serviceref, dest):
	...
	src = isinstance(serviceref, str) and serviceref + ".ts" or normpath(serviceref.getPath())
	...
	if isinstance(serviceref, str) or not serviceref.flags & eServiceReference.mustDescent:
		...
		srcBase = plitext(src)[0]
		baseName = split(srcBase)[1]
		eitName = srcBase + '.eit'
		if exists(eitName):
			moveList.append((eitName, pathjoin(dest, baseName + '.eit')))
		baseName = split(src)[1]
		for ext in ('.ap', '.cuts', '.meta', '.sc'):
			candidate = src + ext
			if exists(candidate):
				moveList.append((candidate, pathjoin(dest, baseName + ext)))
	return moveList
			...
			try:
				...
				if item[0].flags & eServiceReference.mustDescent: # directory
					...
					for ext in ('.eit', self.extension + '.cuts', self.extension):
						newfilename = pathjoin(path, newbasename) + ext
						oldfilename = pathjoin(path, oldbasename) + ext
					...
					try:
						...
						for root, dirnames, filenames in walk(cur_path):
							...
							for fn in filenames:
								...
								rename(pathjoin(root, fn), pathjoin(trashroot, fn))
							for dn in dirnames:
								print("MkDir", pathjoin(trashroot, dn))
								mkdir(pathjoin(trashroot, dn))
